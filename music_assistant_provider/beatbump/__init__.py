...
class BeatbumpProvider(MusicProvider):
    @property
    def is_streaming_provider(self) -> bool: return True
    @property
    def supported_media_types(self) -> set[MediaType]: return {MediaType.TRACK}
    @property
    def base_url(self) -> str: return str(self.get_setup_value(CONF_BASE_URL)).rstrip("/")
    async def _get_json(self, path: str, **params: str) -> dict[str, Any]:
        async with self.mass.http_session.get(f"{self.base_url}{path}", params=params, timeout=30) as response:
            ...
            if not isinstance(data, dict): raise ValueError("Beatbump returned an unexpected response")
            return data
    async def _search_songs(self, query: str) -> list[dict[str, Any]]:
        payload = await self._get_json("/api/v1/search.json", q=query, filter="songs"); items=[]
        for shelf in payload.get("results") or []:
            if isinstance(shelf, dict): items.extend(x for x in shelf.get("contents") or [] if isinstance(x, dict))
        return items
    async def search(self, search_query: str, media_types: list[MediaType], limit: int = 5) -> SearchResults:
        result=SearchResults()
        if MediaType.TRACK in media_types:
            items=await self._search_songs(search_query); result.tracks=[t for item in items[:limit] if (t:=self._parse_track(item))]
        return result
    async def browse(self, path: str) -> Sequence[MediaItemType | BrowseFolder]:
        base=f"{self.instance_id}://"; subpath=path.split("://",1)[1].strip("/") if "://" in path else ""
        if not subpath:
            return [BrowseFolder(item_id="home", provider=self.instance_id, path=f"{base}home", name="Beatbump Home")]
        if subpath != "home": return []
        payload=await self._get_json("/api/v1/home.json"); result=[]; seen=set()
        for carousel in payload.get("carousels") or []:
            ...
            for item in carousel.get("contents") or []:
                ...
                vid=_first_string(item,"videoId")
                if not vid or vid in seen: continue
                track=self._parse_track(item)
                if track: seen.add(vid); result.append(track)
        return result
    ...
    def _parse_track(self,data:dict[str,Any],position:int=0)->Track|None:
        vid=_first_string(data,"videoId"); title=_first_string(data,"title","name","text")
        if not vid or not title:return None
        artist_name,artist_id=_extract_artist(data); artists=UniqueList(); mapping=self._artist_mapping(artist_name,artist_id) if artist_name else None
        if mapping:artists.append(mapping)
        track=Track(item_id=vid,provider=self.instance_id,name=title,duration=_extract_duration(data),provider_mappings={self._provider_mapping(vid,True)},position=position or None)
        if artists:track.artists=artists
        return track
    async def get_artist(self,prov_artist_id:str)->Artist:
        payload=await self._get_json(f"/api/v1/artist/{prov_artist_id}"); header=payload.get("header") or {}; name=_first_string(header,"name","title") or prov_artist_id
    ...
    async def get_track(self,prov_track_id:str)->Track:
        payload=await self._get_json("/api/v1/player.json",videoId=prov_track_id); details=payload.get("videoDetails") or {}; title=str(details.get("title") or prov_track_id); duration=_as_int(details.get("lengthSeconds")); artist_mapping=None
        try:
            for candidate in await self._search_songs(title):
                if _first_string(candidate,"videoId")!=prov_track_id:continue
                artist_name,artist_id=_extract_artist(candidate); artist_mapping=self._artist_mapping(artist_name,artist_id) if artist_name else None; break
    ...
    async def get_stream_details(self,item_id:str,media_type:MediaType)->StreamDetails:
        ...
        formats=(payload.get("streamingData") or {}).get("adaptiveFormats") or []; audio=[x for x in formats if isinstance(x,dict) and x.get("url") and "audio" in str(x.get("mimeType","")).lower()]
        if not audio:raise ValueError(f"Beatbump returned no audio stream for {item_id}")
        aac=[x for x in audio if _as_int(x.get("itag"))==140]; selected=aac[0] if aac else max(audio,key=lambda x:_as_int(x.get("bitrate")) or 0); url=str(selected["url"]); mime=str(selected.get("mimeType","")).lower(); ctype=ContentType.M4A if "audio/mp4" in mime else ContentType.WEBM if "audio/webm" in mime else ContentType.UNKNOWN; expiration=3600; expire=parse_qs(urlparse(url).query).get("expire",[None])[0]
        if expire:
            try:expiration=max(60,int(expire)-int(time.time()))
            except (TypeError,ValueError):pass
        duration_ms=_as_int(selected.get("approxDurationMs")); self.logger.info("Beatbump playback %s -> itag=%s mime=%s",item_id,selected.get("itag"),selected.get("mimeType"))
        return StreamDetails(provider=self.instance_id,item_id=item_id,media_type=MediaType.TRACK,stream_type=StreamType.HTTP,path=url,audio_format=AudioFormat(content_type=ctype),duration=(duration_ms/1000) if duration_ms else None,can_seek=True,allow_seek=True,expiration=expiration,is_realtime=True)
