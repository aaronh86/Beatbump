...
from music_assistant_models.media_items import AudioFormat, ItemMapping, ProviderMapping, SearchResults, Track, UniqueList
...
class BeatbumpProvider(MusicProvider):
    ...
    async def _get_json(self, path: str, **params: str) -> dict[str, Any]:
        async with self.mass.http_session.get(f"{self.base_url}{path}", params=params, timeout=20) as response:
    ...
    async def search(self, search_query: str, media_types: list[MediaType], limit: int = 5) -> SearchResults:
        ...
        payload = await self._get_json("/api/v1/search.json", q=search_query, filter="songs")
        tracks: list[Track] = []
        seen: set[str] = set()
        for shelf in payload.get("results") or []:
            ...
            for candidate in shelf.get("contents") or []:
                if not isinstance(candidate, dict) or candidate.get("type") not in (None, "songs"):
                    continue
    ...
    def _track_from_search(self, video_id: str, title: str, data: dict[str, Any]) -> Track:
        artist, artist_id = _extract_artist(data)
        duration = _extract_duration(data)
        artists = UniqueList([ItemMapping(media_type=MediaType.ARTIST, item_id=artist_id or artist, provider=self.instance_id, name=artist)]) if artist else UniqueList()
        return Track(
            item_id=video_id,
            ...
            duration=duration,
            artists=artists,
            provider_mappings={ProviderMapping(item_id=video_id, provider_domain=self.domain, provider_instance=self.instance_id, available=True, audio_format=AudioFormat(content_type=ContentType.UNKNOWN))},
        )
    ...
    async def get_track(self, prov_track_id: str) -> Track:
        ...
        return self._track_from_search(prov_track_id, title, {"artist": artist, "duration": duration})
    ...
    async def get_stream_details(self, item_id: str, media_type: MediaType) -> StreamDetails:
        ...
        audio_formats = [item for item in formats if isinstance(item, dict) and item.get("url") and "audio" in str(item.get("mimeType", "")).lower()]
        if not audio_formats:
            raise ValueError(f"Beatbump returned no audio stream for {item_id}")

        # Prefer YouTube's AAC-LC/M4A stream (itag 140). It is broadly supported
        # by Music Assistant/ffmpeg and avoids relying on WebM/Opus handling.
        aac_140 = [item for item in audio_formats if _as_int(item.get("itag")) == 140]
        selected = aac_140[0] if aac_140 else max(audio_formats, key=lambda item: _as_int(item.get("bitrate")) or 0)
        mime = str(selected.get("mimeType", "")).lower()
        content_type = ContentType.M4A if "audio/mp4" in mime or "m4a" in mime else ContentType.WEBM if "audio/webm" in mime else ContentType.UNKNOWN
        duration_ms = _as_int(selected.get("approxDurationMs"))
        self.logger.debug(
            "Beatbump stream %s: itag=%s mime=%s bitrate=%s duration_ms=%s",
            item_id, selected.get("itag"), selected.get("mimeType"), selected.get("bitrate"), duration_ms,
        )
